# 🛠️ Contributing to Jirnyak/Population_game
...
> Technology Foundation: `JavaScript / Canvas 2D / Web Audio API`
...
Population Game Demographic & Economic Multi-Agent Simulator is engineered for maximum performance, deterministic state transitions, and zero computational slop. All contributions must respect existing subsystem boundaries and data flows:

```mermaid
graph LR
    A[Age-Cohort Arrays] --> B[Gompertz-Makeham Mortality Solver]
    B --> C[Labor Market Supply Clearing]
    C --> D[Canvas 2D Population Pyramids]
```
...
### 1. Yearly Cohort Migration
* **Formal Requirement**: Age cohorts must transition exactly once per annual simulation cycle.
...
### 2. Gompertz-Makeham Mortality
* **Formal Requirement**: Natural death probabilities must strictly follow mathematical mortality laws.
* **Verification Protocol**: Automated unit test assertion + mathematical boundary check.
* **Failure Mode**: Immediate build rejection; PR cannot be approved without meeting this invariant.
...
* Primary Runtime: `JavaScript / Canvas 2D / Web Audio API`
...
```typescript
// ✅ CORRECT: Gompertz-Makeham Mortality Rate
function computeMortality(age: number, alpha: number, beta: number, lambda_: number): number {
    return lambda_ + alpha * Math.exp(beta * age);
}
```
